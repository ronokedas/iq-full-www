"""Executável do backtest isolado S5-M5."""
...
def _markdown_table(frame: pd.DataFrame) -> str:
    ...
    for values in frame.itertuples(index=False, name=None):
        ...
        for column, value in zip(columns, values):
            formatted.append(f"{value:.2%}" if column == "taxa_acerto" else str(value))
...
def run() -> Path:
    ...
    if trades.empty:
        raise RuntimeError("Nenhuma operação S5-M5 encontrada")
    ...
    lines = ["# Backtest isolado — S5-M5", "", "Dados: parquets M1 locais. Nenhum componente do robô de produção foi alterado.", "", "## Geral", "", _markdown_table(overall), "", "## Por ativo", "", _markdown_table(by_asset), "", "## Por hora (Brasília)", "", _markdown_table(by_hour), "", "## Cobertura", "", f"Ativos: {len(coverage)} | gaps M1: {sum(item['gaps'] for item in coverage)} | operações sem vela de resultado: {sum(item['skipped_no_result'] for item in coverage)}."]

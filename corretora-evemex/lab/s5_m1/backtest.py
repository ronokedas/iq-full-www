"""Executável do backtest isolado S5-M1."""
...
def _markdown_table(frame: pd.DataFrame) -> str:
    ...
    for values in frame.itertuples(index=False, name=None):
        rows.append("| " + " | ".join(f"{value:.2%}" if column == "taxa_acerto" else str(value) for column, value in zip(columns, values)) + " |")
...
def run() -> Path:
    ...
    if trades.empty:
        raise RuntimeError("Nenhuma operação S5-M1 encontrada")
    ...
    lines = [
        "# Backtest isolado — S5-M1", "", "Dados: parquets M1 locais. Nenhum componente do robô de produção foi alterado.",
        "", "## Geral", "", _markdown_table(overall), "", "## Por ativo", "", _markdown_table(by_asset),
        "", "## Por hora (Brasília)", "", _markdown_table(by_hour), "", "## Cobertura", "",
        f"Ativos: {len(coverage_frame)} | candles duplicados: {int(coverage_frame.duplicates.sum())} | gaps M1: {int(coverage_frame.gaps.sum())} | comandos: {int(coverage_frame.commands.sum())} | primeiros toques invalidados: {int(coverage_frame.first_touches_invalidated.sum())} | operações sem vela de resultado: {int(coverage_frame.skipped_no_result.sum())}.",
    ]

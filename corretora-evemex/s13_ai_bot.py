"""Robô S13 (Pavios de Rejeição) com filtro de IA na Evemex.

Estratégia: 3 velas da mesma cor com pavio na direção oposta >= 40% do range.
...
class JsonlLogger:
    def __init__(self, directory: Path = Path("logs")) -> None:
        ...
        self.path = directory / f"trades-s13-{datetime.now():%Y-%m-%d}.jsonl"
...
def upper_wick_ratio(candle: Candle) -> float:
    """Pavio superior como fração do range (0..1)."""
    rng = candle.high - candle.low
    if rng <= 0:
        return 0.0
    return max(0.0, min(1.0, (candle.high - max(candle.open, candle.close)) / rng))


def lower_wick_ratio(candle: Candle) -> float:
    """Pavio inferior como fração do range (0..1)."""
    rng = candle.high - candle.low
    if rng <= 0:
        return 0.0
    return max(0.0, min(1.0, (min(candle.open, candle.close) - candle.low) / rng))


def detect_s13(candles: list[Candle]) -> tuple[str, float, float, float] | None:
    """Detecta o padrão S13 (Pavios de Rejeição) nas 3 últimas velas.

    Retorna (direção, p1, p2, p3) se o padrão for confirmado.
    """
    if len(candles) < 3:
        return None
    recent = sorted(candles, key=lambda candle: candle.from_ts)[-3:]
    colors = [candle_color(candle) for candle in recent]
    if colors[0] not in {"GREEN", "RED"} or len(set(colors)) != 1:
        return None

    if colors[0] == "GREEN":
        # 3 velas verdes com pavio superior >= 40% => PUT
        ratios = [upper_wick_ratio(candle) for candle in recent]
        if all(ratio >= 0.40 for ratio in ratios):
            return "DOWN", ratios[0], ratios[1], ratios[2]
    else:
        # 3 velas vermelhas com pavio inferior >= 40% => CALL
        ratios = [lower_wick_ratio(candle) for candle in recent]
        if all(ratio >= 0.40 for ratio in ratios):
            return "UP", ratios[0], ratios[1], ratios[2]
    return None


class S13FeatureBuilder:
...
class S13AIBot:
    ...
    def run(self, *, once: bool = False) -> None:
        print(
            f"Robô S13+IA ativo | conta={self.client.selected_account.mode if self.client.selected_account else '?'} "
    ...
    def build_signals(self, batch: dict[str, list[Candle]]) -> list[dict[str, Any]]:
        """Detecta sinais S13 e aplica o filtro de IA.
        ...
        for symbol in self.symbols:
            ...
            # Detectar S13 nas últimas 3 velas fechadas
            signal_info = detect_s13(closed)
            if signal_info is None:
                continue

            direction, p1, p2, p3 = signal_info
            self._total_signals += 1

            # Construir features para o modelo
            builder = S13FeatureBuilder(self.history[symbol])
            features = builder.build()
            if features is None:
                continue

            # Adicionar features específicas do padrão S13
            features["s13_p1"] = p1
            features["s13_p2"] = p2
            features["s13_p3"] = p3
        ...
        with ThreadPoolExecutor(max_workers=min(12, len(selected))) as executor:
            ...
            for signal in selected:
                ...
                future = executor.submit(
                    ...
                    client_request_id=f"req_s13_{signal['symbol'].lower()}_{signal['candle_from']}",
                )
    ...
    def _prefetch_candidates(self, batch: dict[str, list[Candle]]) -> list[str]:
        ...
        for symbol in self.symbols:
            ...
            closed = [candle for candle in candles if candle.from_ts < current_minute]
            if detect_s13(closed) is not None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        default="corretora-evemex/signal_filter_s13.pkl",

"""Gera o dataset de treinamento para o filtro de IA da Estratégia S13.

As features calculadas aqui são IDÊNTICAS às usadas pelo bot em produção
(s13_ai_bot.py -> S13FeatureBuilder.build()), garantindo consistência
...
def calculate_features(df: pd.DataFrame) -> pd.DataFrame:
    """Calcula as features espelhando exatamente o S13FeatureBuilder do bot.
...
def detect_s13_signals(df: pd.DataFrame) -> list[dict]:
    """Detecta sinais da estratégia S13 - Pavios de Rejeição.

    O padrão é confirmado no fechamento de v3. O timestamp do sinal é o
    from_ts de v3 (última vela fechada), igual ao bot em produção.
    """
    signals: list[dict] = []

    # Precisamos de pelo menos 4 velas (3 para o sinal + 1 para o resultado)
    for i in range(3, len(df) - 1):
        # Velas do sinal: v1, v2, v3 (padrão confirmado no fechamento de v3)
        v1 = df.iloc[i - 3]
        v2 = df.iloc[i - 2]
        v3 = df.iloc[i - 1]
        ...
        # Lógica S13 PUT (Rejeição de Alta)
        # 3 velas verdes com pavios superiores >= 40%
        if v1["close"] > v1["open"] and v2["close"] > v2["open"] and v3["close"] > v3["open"]:
            p1 = (v1["high"] - v1["close"]) / (v1["high"] - v1["low"]) if (v1["high"] - v1["low"]) > 0 else 0
            p2 = (v2["high"] - v2["close"]) / (v2["high"] - v2["low"]) if (v2["high"] - v2["low"]) > 0 else 0
            p3 = (v3["high"] - v3["close"]) / (v3["high"] - v3["low"]) if (v3["high"] - v3["low"]) > 0 else 0

            if p1 >= 0.4 and p2 >= 0.4 and p3 >= 0.4:
                # Sinal de PUT
                target = 1 if v_result["close"] < v_result["open"] else 0  # WIN se a próxima for vermelha
                signals.append({
        ...
        # Lógica S13 CALL (Rejeição de Baixa)
        # 3 velas vermelhas com pavios inferiores >= 40%
        elif v1["close"] < v1["open"] and v2["close"] < v2["open"] and v3["close"] < v3["open"]:
            p1 = (v1["open"] - v1["low"]) / (v1["high"] - v1["low"]) if (v1["high"] - v1["low"]) > 0 else 0
            p2 = (v2["open"] - v2["low"]) / (v2["high"] - v2["low"]) if (v2["high"] - v2["low"]) > 0 else 0
            p3 = (v3["open"] - v3["low"]) / (v3["high"] - v3["low"]) if (v3["high"] - v3["low"]) > 0 else 0

            if p1 >= 0.4 and p2 >= 0.4 and p3 >= 0.4:
                # Sinal de CALL
                target = 1 if v_result["close"] > v_result["open"] else 0  # WIN se a próxima for verde
                signals.append({
                    "timestamp": v3["from_ts"],
                    "direction": "CALL",
                    "target": target,
...
def generate_dataset() -> None:
    ...
    output_path = "corretora-evemex/ml_dataset_s13.csv"
    ...
    for file in files:
        ...
        # 1. Calcula Features (mesmas do bot em produção)
        df_feat = calculate_features(df)

        # 2. Detecta Sinais S13
        s13_signals = detect_s13_signals(df)
    ...
    if all_samples:
        ...
        print(f"\n✅ Dataset S13 gerado com sucesso!")
        print(f"📊 Total de amostras: {len(dataset)}")
        print(f"🎯 Distribuição do Target:\n{dataset['target'].value_counts(normalize=True)}")
    else:
        print("\n⚠️ Nenhum sinal S13 encontrado nos dados fornecidos.")


if __name__ == "__main__":
    generate_aligned_dataset("s13")
